...
//Define this to link all static lib with this project.
#ifdef GT_BUILD_LIB
//Binding all Export (DLL) methods to static library.
#pragma comment (lib, "User32.lib") //For User Interface Functions.
#pragma comment (lib, "Kernel32.lib") //For Core Memory,I/O Fucntions.
#pragma comment (lib, "Advapi32.lib") // For security calls 
#pragma comment (lib, "Psapi.lib") // For Process Modules. 
...
/*Enum for OPCODE type*/
	...
	DWORD GT_GetJmpBackAddress(LPVOID, DWORD, LPSTR);
#endif
	...
	/*Semi-private Tool for Checking file.*/
	BOOL GT_FileExist(LPCSTR);
	...
	static LPVOID GT_MemAlloc(DWORD, SIZE_T);
	static BOOL GT_MemFree(LPVOID);
