...
LPSTR GT_GetProcessModule(LPSTR module_name)
{
            ...
            if (GetModuleFileNameEx(game_handle, module_handles[i], module_path, sizeof(module_path) / sizeof(char)))
            {
                if (lstrlen(module_name) > 1)
                {
                    if (strstr(module_path, module_name) != GT_NULL)
                    {
                        ...
                        wsprintf(modules_list + lstrlen(modules_list), "\t%s (0x%08X)\n", module_name, module_handles[i]);
                        return modules_list;
                    }
                }
    ...
    if (GT_IsLogEnabled())
        GT_AddLog("%s -> returned ModuleList count %d \n", FUNC_NAME, n_modules);
    ...
    gt_try
    {
        ...
        do
        {
            if (te32.th32OwnerProcessID == pid)
            {
                ...
                h_thread = OpenThread(THREAD_SUSPEND_RESUME, FALSE, te32.th32ThreadID);
                if (h_thread == GT_NULL)
                {
                    gt_throw(GT_GetError());
                }
...
DWORD GT_GetJmpBackAddress(LPVOID address, DWORD length, LPSTR jmp_back_buf)
{
    ...
    if (address != GT_NULL && length > 5)
    {
        jmp_back_addr = (DWORD)address + length;
    }

    if (jmp_back_buf != GT_NULL)
    {
        lstrcpyA(jmp_back_buf, "jmp[jmp_back_addr]");
    }
...
{
    MessageBox((HWND)GT_NULL, info_msg, "INFO!", MB_ICONINFORMATION);
}
...
{
    if (GT_IsPrivateMethod(gt_private_method, FUNC_NAME, LINE_NO))
        ShellExecute(GT_NULL, "open", "cmd.exe", cmd, GT_NULL, SW_HIDE);
}

static void GT_ShellExecAdmin(LPCSTR cmd)
{
    if (GT_IsPrivateMethod(gt_private_method, FUNC_NAME, LINE_NO))
        ShellExecute(GT_NULL, "runas", "cmd.exe", cmd, GT_NULL, SW_HIDE);
}
...
static BOOL GT_IsElevatedProcess()
{
    BOOL is_elevated = FALSE;
    HANDLE token = GT_NULL;
    if (GT_IsPrivateMethod(gt_private_method, FUNC_NAME, LINE_NO))
    {
...
static LPVOID GT_MemAlloc(DWORD dw_flag, SIZE_T sz_size)
{
...
static BOOL GT_MemFree(LPVOID gt_mem_block)
{
